...
struct RequestRowView {
    ...
    /// Whether the upstream's reported tier DISAGREES with the tier requested.
    ///
    /// Deliberately not called "downgraded" and not surfaced as one: Codex reports
    /// `service_tier: "default"` even for turns it genuinely serves at priority
    /// (openai/codex#30413, still open). This deployment's own traffic shows priority-requested
    /// turns running ~1.3-1.9x the tokens/sec of standard ones while 100% of them report
    /// `default`, so the disagreement says something is wrong with the LABEL, not the service.
    /// Kept as a diagnostic; no UI presents it as a refusal.
    service_tier_reported_mismatch: bool,
...
fn live_value(rs: &crate::runtime_settings::RuntimeSettings, key: &str) -> String {
    match key {
        ...
        "debug_trace" => rs.debug_trace().to_string(),
        "capacity_ride_out_secs" => rs.capacity_ride_out_secs().to_string(),
