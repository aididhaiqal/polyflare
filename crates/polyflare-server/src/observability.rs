...
pub struct RequestLog {
    ...
    /// Live-usage-cost-capture Task 4: a per-request correlation id, generated by ingress
    /// (`format!("{:032x}", rand::random::<u128>())` — 128 random bits, uuid-equivalent
    /// uniqueness, content-free by construction: not derived from any request/response data).
...
mod tests {
    ...
    /// persisted `RequestLogRecord` unchanged — the correlation key the (later) stream-wrapper task
    /// needs to call `RequestLogRepo::update_usage` against the SAME row `record()` inserted.
    #[test]
    fn record_carries_request_id_through_to_the_persisted_record() {
        ...
        let record = log.record(100);
        assert_eq!(record.request_id.as_deref(), Some("test-id"));
    }
