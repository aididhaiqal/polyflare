...
/// [`resolve_owner_affine_ws_account_with_capability`] with accounts ruled out: the session's
/// current owner that has just proven unable to serve THIS turn (an upstream overload before any
/// output), plus every account already tried for the same turn — so a turn moves forward
/// through the fleet instead of bouncing back to one it just left. An excluded owner is treated
/// exactly as an ineligible owner — the same never-stranded fallback to any other eligible
/// account — and excluded accounts are removed from that fallback pool too. An empty slice is
/// the plain resolution. Fails with the usual `503` when no OTHER account is eligible, so a
/// single-account fleet keeps its in-place behaviour.
...
async fn resolve_owner_affine_account_inner(
    ...
    // An excluded account is absent from the pool outright, so an owner that matches one takes
