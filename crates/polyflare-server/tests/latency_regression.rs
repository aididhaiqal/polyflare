//! Latency-regression gate (M5 MVP CI gate #3 of 4).
//!
//! Guards against the single most likely latency regression: PolyFlare accidentally buffering
//! the streaming upstream response instead of relaying it chunk-by-chunk. Check 1 is the primary,
//! machine-independent assertion (relative timing against an injected inter-chunk gap); Check 2 is
//! a secondary, deliberately generous absolute-overhead budget that only catches gross (e.g. 10x)
//! regressions, not an SLA.
//!
