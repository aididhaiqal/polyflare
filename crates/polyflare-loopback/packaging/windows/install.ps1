...
$Arguments = "`"$Destination`" --windows-service --upstream-origin `"$UpstreamOrigin`" --listen 127.0.0.1:8000"
...
for ($Attempt = 0; $Attempt -lt 30; $Attempt++) {
    try {
        $Health = Invoke-RestMethod -TimeoutSec 4 -Uri "http://127.0.0.1:8000/_polyflare-loopback/health"
