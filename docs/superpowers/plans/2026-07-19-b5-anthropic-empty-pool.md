...
- **The wedge fix is sacred.** Do NOT touch `ObservingStream::poll_next` (`watchdog.rs`) or its logic. The empty-pool paths are wedge-neutral BY CONSTRUCTION: both Anthropic handlers use `NoopContinuity` (always `Disarmed`), and the empty-pool branch is strictly *pre-selection* — no `ObservingStream` is ever constructed until a splice succeeds, and the splice re-uses the same `NoopContinuity`. Preserve that; do not arm continuity anywhere.
